...
program Code_103
  ...
  real(8),dimension (1:nx-1,1:ny-1,1:nz-1)    ::   fbox,fboy,fboz,divv
...
122 format (11(1x,e15.7)) 
   ...
   if (START) then
     ...
     call proprety(ro,rodrop,roair,rosolid,rocon,gap)
   ...
   if (START) then
     ...
     write(65,111) 'variables="Time(Sec)","tether1","Tether2","tether3"'


     write(75,111) 'variables="Time(Sec)","Fx1(N)","Fx2(N)","Fz1(N)","Fz2(N)","MT(N.m)","MD(N.m)","ML(N.m)"'
     ...
     OPEN(65,file='tetherforce.plt',POSITION='APPEND',status='old')
     ...
     call Teth_Sec_Sav
      ...
      do torder=1,2
         ...
         do while (numimp.lt.Iterate) 
           ...
           call proprety(ro,rodrop,roair,rosolid,rocon,gap)
           call proprety(miuv,miudrop,miuair,miusolid,miucon,gap)
           call advection(advectu,advectv,advectw)
           call viscoset(miuv,ro,Tx,Ty,Tz)
           call AddForce(fbox,fboy,fboz,ro,NumImp,tp,Iterate,torder)   
          ...
          ! call boundarycond()  !! since the  solid has intersection with boundary 
          ...
          !call contactangle(uext,vext,wext,gap) 
          !call levelset2(uext,vext,wext,0.25*dt)
...
write(65,2012)  tp*dt,KTeth* ( max(Lzero(1),Lnew(1))-Lzero(1) ),KTeth* ( max(Lzero(2),Lnew(2))-Lzero(2) ), &
                 &  KTeth* ( max(Lzero(3),Lnew(3))-Lzero(3) )
...
& ( -advectu(i,j,k)-(  1/(  0.5*(ro(i,j,k)+ro(i-1,j,k))  )   )*( p(i,j,k)-p(i-1,j,k) )/(x(i)-x(i-1))+Tx(i,j,k)+gx )
...
MDrag=MDrag+(Z(k)-ZFree)*(0.5*(ro(i,j,k)+ro(i-1,j,k))*0.5* (Ib_Solid(i,j,k)+Ib_Solid(i-1,j,k))* &
& (x(i)-x(i-1))*( 0.5*(y(j)+y(j+1))-0.5*(y(j)+y(j-1)) )*( 0.5*(z(k)+z(k+1))-0.5*(z(k)+z(k-1)) )*  & 
& ( -advectu(i,j,k)-(  1/(  0.5*(ro(i,j,k)+ro(i-1,j,k))  )   )*( p(i,j,k)-p(i-1,j,k) )/(x(i)-x(i-1))+Tx(i,j,k)+gx ) )

MLift=MLift+(x(i)-xbar)*(0.5*(ro(i,j,k)+ro(i,j,k-1))*0.5*(Ib_Solid(i,j,k)+Ib_Solid(i,j,k-1))* &
& (   0.5*(x(i)+x(i+1))-0.5*(x(i)+x(i-1)) )*( 0.5*(y(j)+y(j+1))-0.5*(y(j)+y(j-1)) )*(z(k)-z(k-1))*  &
& (   -advectw(i,j,k)-(  1/(  0.5*(ro(i,j,k)+ro(i,j,k-1)))   )*( p(i,j,k)-p(i,j,k-1) )/(z(k)-z(k-1))+Tz(i,j,k)+gz ) )
...
!write(75,2013)  tp*dt,Drag,(sumv-sumvold)/dt,Lift, &
!             &        (sumIx-SumIxold)/dt,(sumIy-SumIyold)/dt,(sumIz-SumIzold)/dt

write(75,2013)  tp*dt,Drag,(sumu-sumuold)/dt,Lift,(sumw-sumwold)/dt,(sumIy-sumIyold)/dt,MDrag,MLift
                     
2013  format (8(1x,e15.7))
call flush (75)
       ...
       do k=1,nz-1 
         do j=1,ny-1 
           do i=1,nx-1
             ...
             &            0.5*(w(i,j,k)+w(i,j,k+1)),phi(i,j,k),ro(i,j,k),p(i,j,k),Ib_Solid(i,j,k),divv(i,j,k)
...
Q(I,J,K)=(  (U(I+1,J,k)-U(I,J,k))/( 0.5*(x(i+1)+x(i)  )-0.5*(x(i)  +x(i-1)) )+&
 ...
 Cof(:,:,:,8)=Q  (:,:,:)
  ...
  subroutine SORpoison(beta,pdif,p,maxSOR,COF)
 use M_General,only: nx,ny,nz,TEST
...
subroutine contactangle(uext,vext,wext,gap)


use M_General,          only : nx,ny,nz,x,y,z,phi,TEST
...
550 format (11(1x,e15.7)) 
 ...
 subroutine levelset2(uext,vext,wext,dt) 
use M_General,          only: nx,ny,nz,x,y,z,phi,TEST
...
subroutine proprety(pro,prodrop,proair,prosolid,procon,gap)
use M_General,             only: nx,ny,nz,pi,phi
use M_Solid,               only: Ib_Solid,Ic_Solid
implicit none 

integer i,j,k 
dOUBLE PRECISION gap,prodrop,prosolid,proair,procon,GapN
