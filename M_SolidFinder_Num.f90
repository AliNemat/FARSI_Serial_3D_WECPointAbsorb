Module M_SolidFinder_Num
  ...
  contains
    ...
    subroutine Solid_Box_Mark(pointO,Width,Ib_SolidFindB_S,npointsO)
      implicit none 
      double precision,intent(in),dimension(npointsO,3)        :: pointO
      real(8)         ,intent(out),dimension(0:nx,0:ny,0:nz)   :: Ib_SolidFindB_S
      integer         ,intent(in)                              :: npointsO
      real(8)         ,intent(in)                              :: Width

      real(8)         ,dimension(0:nx,0:ny,0:nz)               :: Ib_SolidFindB
      ...
      !call FractionSmoother_2D()
      call FractionSmoother_3D(Ib_solidFindB,Ib_SolidFindB_S)
...
return 
    ...
    Subroutine FractionSmoother_3D(Ib_solidFindB,Ib_SolidFindB_S)
      implicit none
      real(8),intent(in) ,dimension(0:nx,0:ny,0:nz)                 :: Ib_SolidFindB
      real(8),intent(out),dimension(0:nx,0:ny,0:nz)                 :: Ib_SolidFindB_S
      integer i,j,k
      !! imported from Paris Code !!
      ...
      Ib_SolidFindB_S(:,:,:)=0  !! should be here for boder points
      ...
      if (smooth) then
        ...
        do i=1,nx-1 
          do j=1,ny-1 
            do k=1,nz-1 
              Ib_SolidFindB_S(i,j,k) = &
              &  8d0/64*  Ib_SolidFindB(i,j,k) + & 
                 4d0/64*( Ib_SolidFindB(i-1,j,k) + Ib_SolidFindB(i,j-1,k) + Ib_SolidFindB(i,j,k-1) + &
                          Ib_SolidFindB(i+1,j,k) + Ib_SolidFindB(i,j+1,k) + Ib_SolidFindB(i,j,k+1) ) + &
                 2d0/64*( Ib_SolidFindB(i+1,j+1,k) + Ib_SolidFindB(i+1,j-1,k) + Ib_SolidFindB(i-1,j+1,k) + Ib_SolidFindB(i-1,j-1,k) + &
                          Ib_SolidFindB(i+1,j,k+1) + Ib_SolidFindB(i+1,j,k-1) + Ib_SolidFindB(i-1,j,k+1) + Ib_SolidFindB(i-1,j,k-1) + &
                          Ib_SolidFindB(i,j+1,k+1) + Ib_SolidFindB(i,j+1,k-1) + Ib_SolidFindB(i,j-1,k+1) + Ib_SolidFindB(i,j-1,k-1) ) + &
                 1d0/64*( Ib_SolidFindB(i+1,j+1,k+1) + Ib_SolidFindB(i+1,j+1,k-1) + Ib_SolidFindB(i+1,j-1,k+1) + Ib_SolidFindB(i+1,j-1,k-1) +  &
                          Ib_SolidFindB(i-1,j+1,k+1) + Ib_SolidFindB(i-1,j+1,k-1) + Ib_SolidFindB(i-1,j-1,k+1) + Ib_SolidFindB(i-1,j-1,k-1) )
             enddo
           enddo
         enddo

      else
        Ib_SolidFindB_S(:,:,:)=Ib_SolidFindB(:,:,:)
      endif
