...
Module M_Solid
  ...
  use M_General,     only: dt,nx,ny,nz,pi,ChV,Bp,x,y,z,rodrop,Froude,fileplace
  ...
  real(8)                                     ::   Len_Cyl_1,r_Cyl_1,Len_Cyl_2,r_Cyl_2
  real(8),dimension (0:nx,0:ny,0:nz)          ::   Ib_Cyl_1,Ib_Cyl_2
...
!! whole body !!
  ...
  Real(8)                                     ::   rocon,rosolid,miusolid,miucon
  real(8)                                     ::   Solid_S_Ini,Solid_H_Ini,Solid_tetang_Ini
  real(8)                                     ::   Solid_Mas_Anal,Solid_Bot,Solid_ResB_Anal
...
!      print *, 'Hull mass=',M_Cyl/1000,'Tonnes Theoritical'
      ...
      Solid_Mas_Anal= (  pi*(r_Cyl_1**2 )*len_Cyl_1+             &
                   & 4/3*pi*(r_Sphe_1**3)*0.5+                  &
                   & 4/3*pi*(r_Sphe_2**3)*0.5+                  &
                   & Len_Box_1*H_Box_1*W_Box_1+                 &
                   & Len_Box_2*H_Box_2*W_Box_2      )*rosolid + &
                   &(    pi*(r_Cyl_1**2 )*len_Cyl_1+            &
                   & 4/3*pi*(r_Sphe_1**3)*0.5+                  &
                   & 4/3*pi*(r_Sphe_2**3)*0.5       )*rocon      

      print*,'mass=', Solid_Mas_Anal/1000 ,'tonnes Theoritical'

      Solid_ResB_Anal= (  pi*(r_Cyl_1**2 )*len_Cyl_1+                       &  
                     & 4/3*pi*(r_Sphe_1**3)*0.5+                            &
                     & 4/3*pi*(r_Sphe_2**3)*0.5+                            &
                     & Len_Box_1*(H_Box_1-r_Cyl_1)*W_Box_1+                 &
                     & Len_Box_2*(H_Box_2-r_cyl_1)*W_Box_2      )*rodrop    

      print*,'Reserve buoyancy=', Solid_ResB_Anal/1000 ,'tonnes Theoritical'
    ...
    Subroutine Solid_Finder_Dynamic_Ini(gap)
      ...
      !call Solid_Cyl_Mark(point_Cyl_2,r_Cyl_2,gap,Ib_Cyl_2,2)
      ...
      Ic_solid(:,:,:)=max(IB_Cyl_1(:,:,:),Ib_Sphe_1(:,:,:),Ib_Sphe_2(:,:,:))
